...
const generateBaseString = render => ([`"name":"${render.key}"`, `"type":"${render.nodeName}"`,
  `"path":"${getPath(render)}"`]).join(',');

const stringify = value => (isPlainObject(value) ? complexValue : value);

const formattersByRender = {
  nested: render => [leftCurl, generateBaseString(render), comma, `"children":[${render.children}]`, rightCurl].join(''),
